...
def detect_project_language(project_path: Path) -> Tuple[str, Dict]:
    ...
    indicators = {
        ...
        "go": {"files": ["go.mod", "go.sum"], "extensions": [".go"], "weight": 0},
        "rust": {
        ...
        "csharp": {"files": [], "extensions": [".cs", ".csproj", ".sln"], "weight": 0},
        "bash": {"files": [], "extensions": [".sh"], "weight": 0},
        "dockerfile": {"files": ["Dockerfile"], "extensions": [], "weight": 0},
        "terraform": {"files": [], "extensions": [".tf"], "weight": 0},
        "ansible": {
            "files": ["playbook.yml", "ansible.cfg", "inventory"],
            "extensions": [],
            "weight": 0,
        },
        "json": {"files": [], "extensions": [".json", ".jsonc"], "weight": 0},
        "toml": {"files": ["pyproject.toml"], "extensions": [".toml"], "weight": 0},
        "ini": {
        ...
        "jenkinsfile": {"files": ["Jenkinsfile"], "extensions": [], "weight": 0},
    }
    ...
    for item in project_path.rglob("*"):
        if (
            item.is_file()
            and "_fixtures" not in item.parts
            and (
                not any(p.startswith(".") for p in item.parts)
                or item.name.lower() in allow_hidden_files
            )
        ):
    ...
    stats = {
        ...
        "all_scores": {
            k: v["weight"] for k, v in indicators.items() if v["weight"] > 0
        },
    }
...
class Sandbox:
    ...
    def __init__(self, project_path: str, sandbox_dir: str = None):
        ...
        self.sandbox_dir = (
            Path(sandbox_dir) if sandbox_dir else self.project_path / ".pactfix"
        )
    ...
    def setup(self) -> bool:
        ...
        print(
            f"📋 Detected language: {self.language} (confidence: {self.stats['confidence']})"
        )
        ...
        # Create Dockerfile
        dockerfile_content = LANGUAGE_DOCKERFILES.get(
            self.language, LANGUAGE_DOCKERFILES["generic"]
        )
...
"""
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "-f",
                    str(self.project_copy_dir / self.build_dockerfile_name),
                    str(self.project_copy_dir),
                ],
                capture_output=True,
                text=True,
                timeout=300,
    ...
    def test(self) -> Tuple[bool, str]:
        ...
        cmd = test_commands.get(
            self.language, 'echo "No test command for this language"'
        )
        ok, out = self.run(cmd)
