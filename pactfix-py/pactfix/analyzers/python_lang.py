...
def analyze_python(code: str) -> AnalysisResult:
    ...
    for i, line in enumerate(lines, 1):
        ...
        in_condition_stmt = code_stripped.startswith(
        ...
        if re.match(r"^\s*def\s+\w+\s*\(", current_line):
            ...
            if next_non_empty and not next_non_empty.startswith(('"""', "'''")):
                ...
                if next_idx < len(fixed_lines):
                    ...
                    probe_indent = (
                        probe_indent_match.group(0) if probe_indent_match else ""
                    )
        ...
        if re.match(r'^print\s+["\']', stripped) or re.match(r"^print\s+\w", stripped):
            if not stripped.startswith("print("):
                errors.append(
                    Issue(i, 1, "PY001", "Użyj print() z nawiasami (Python 3)")
                )
        ...
        if re.match(r"^except\s*:", code_stripped):
            warnings.append(
                Issue(i, 1, "PY002", "Unikaj pustego except: - złap konkretne wyjątki")
            )
            if re.match(r"^except\s*:\s*$", code_stripped):
                ...
                if fixed != code_stripped:
                    fixes.append(
                        Fix(
                            i,
                            "Zmieniono except: na except Exception:",
                            code_stripped,
                            fixed,
                        )
                    )
                    fixed_lines[i - 1] = (
                        code_part[: len(code_part) - len(code_part.lstrip())]
                        + fixed
                        + comment_part
                    )
                    current_line = fixed_lines[i - 1]
        ...
        if re.search(r"def\s+\w+\s*\([^)]*=\s*(\[\]|\{\})", stripped):
            warnings.append(
                Issue(i, 1, "PY003", "Mutable default argument - użyj None")
            )
            m = re.match(
                r"^(\s*def\s+\w+\s*\()(?P<args>[^)]*)(\)\s*:.*)$", current_line
            )
            if m:
                args = m.group("args")
                arg_match = re.search(
                    r"(?P<name>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?P<lit>\[\]|\{\})", args
                )
                if arg_match:
                    ...
                    if re.search(
                        rf"^\s*if\s+{re.escape(name)}\s+is\s+None\s*:", next_line or ""
                    ):
                        pass
                    else:
                        def_indent_match = re.match(r"^\s*", current_line)
                        def_indent = (
                            def_indent_match.group(0) if def_indent_match else ""
                        )
                        body_indent = def_indent + "    "
                        for j in range(i, len(fixed_lines)):
                            ...
                            probe_indent = (
                                probe_indent_match.group(0)
                                if probe_indent_match
                                else ""
                            )
                            if len(probe_indent) > len(def_indent):
                                body_indent = probe_indent
        ...
        if in_condition_stmt and ("== None" in code_part or "!= None" in code_part):
            ...
            if fixed_code != code_part:
                ...
                fixes.append(
                    Fix(
                        i,
                        "Zmieniono porównanie do None na is None/is not None",
                        before,
                        after,
                    )
                )
        ...
        if m_type_cmp:
            warnings.append(
                Issue(i, 1, "PY007", "Rozważ isinstance() zamiast type() == ...")
            )
            ...
            if fixed_code != code_part:
                fixes.append(
                    Fix(
                        i,
                        "Zamieniono type(x) == T na isinstance(x, T)",
                        before.strip(),
                        after.strip(),
                    )
                )
        ...
        if in_condition_stmt and (
            re.search(is_not_pat, code_part) or re.search(is_pat, code_part)
        ):
            warnings.append(
                Issue(
                    i, 1, "PY008", 'Nie używaj "is" do porównań z literałami - użyj =='
                )
            )
            fixed_code = code_part
            ...
            if fixed_code != code_part:
                fixes.append(
                    Fix(
                        i,
                        'Zamieniono "is" na == dla literałów',
                        code_part.strip(),
                        fixed_code.strip(),
                    )
                )
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import) and getattr(node, "lineno", None):
                ...
                before_line = (
                    fixed_lines[line_no - 1] if 0 < line_no <= len(fixed_lines) else ""
                )
                warnings.append(
                    Issue(
                        line_no, 1, "PY005", f'Import "{imported}" może być nieużywany'
                    )
                )
                ...
                fixes.append(
                    Fix(
                        line_no,
                        f"Usunięto nieużywany import: {imported}",
                        before_line.strip(),
                        "",
                        edits=[edit],
                    )
