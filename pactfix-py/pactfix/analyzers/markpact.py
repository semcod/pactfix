    ...
    if not blocks:
        warnings.append(
            Issue(
                ...
                1,
                "MP001",
                "Brak bloków markpact:* — plik nie zawiera wykonywalnego kodu",
            )
        )
    ...
    if "file" in kinds_found and "run" not in kinds_found:
        warnings.append(
            Issue(
                ...
                1,
                "MP002",
                "Brak bloku markpact:run — projekt nie ma punktu startowego",
            )
        )
    ...
    if "run" in kinds_found and "deps" not in kinds_found:
        warnings.append(
            Issue(
                ...
                1,
                "MP003",
                "Brak bloku markpact:deps — brak zdefiniowanych zależności",
            )
        )
        ...
        for fix in result.fixes:
            ...
            for edit in fix.edits or []:
                ...
                if "startLine" in mapped_edit:
                    mapped_edit["startLine"] = (
                        body_start_line + int(mapped_edit["startLine"]) - 1
                    )
                if "endLine" in mapped_edit:
                    mapped_edit["endLine"] = (
                        body_start_line + int(mapped_edit["endLine"]) - 1
                    )
                mapped_edits.append(mapped_edit)
            fixes.append(
                Fix(
                    mapped_line,
                    f"[{block_label}] {fix.description}",
                    fix.before,
                    fix.after,
                    edits=mapped_edits,
                )
