...
def analyze_json(code: str) -> AnalysisResult:
    ...
    if re.search(r"\b(True|False|None)\b", fixed_code):
        errors.append(
            Issue(
                1, 1, "JSON004", "Wykryto True/False/None - JSON wymaga true/false/null"
            )
        )
        ...
        if new_code != fixed_code:
            fixes.append(
                Fix(1, "Zamieniono True/False/None na true/false/null", "", "")
            )
            fixed_code = new_code
    ...
    if re.search(r",\s*[\]}]", fixed_code):
        warnings.append(
            Issue(
                1, 1, "JSON005", "Trailing comma w JSON - usuń przecinek przed } lub ]"
            )
        )
        ...
        if dups:
            warnings.append(
                Issue(
                    ...
                    1,
                    "JSON006",
                    f"Duplikaty kluczy w JSON: {', '.join(sorted(set(dups)))}",
                )
            )
