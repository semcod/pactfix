...
def analyze_kubernetes(code: str) -> AnalysisResult:
        ...
        return AnalysisResult(
            ...
            [
                Issue(
                    ...
                    1,
                    "K8S998",
                    "PyYAML not installed - install pactfix[yaml] for full analysis",
                )
            ],
            [],
        )
    ...
    try:
        ...
        return AnalysisResult(
            "kubernetes", code, code, [Issue(1, 1, "K8S999", "Invalid YAML")], [], []
        )
    ...
    for doc in documents:
        ...
        if kind in ("Deployment", "Pod", "StatefulSet", "DaemonSet"):
                ...
                for idx, container in enumerate(containers):
                    ...
                    if image:
                        if ":latest" in image or ":" not in image:
                            # Find image line
                            if container_name:
                                img_line = _find_container_line(
                                    lines, container_name, "image"
                                )
                            else:
                                img_line = _find_container_key_line_by_index(
                                    lines, idx, "image"
                                )
                            if img_line:
                                warnings.append(
                                    Issue(
                                        img_line, 1, "K8S004", "Użyj konkretnego tagu"
                                    )
                                )
                                replacement = _suggest_image_tag(image)
                                if replacement != image:
                                    fixed_lines[img_line - 1] = fixed_lines[
                                        img_line - 1
                                    ].replace(image, replacement)
                                    fixes.append(
                                        Fix(
                                            img_line,
                                            "Zmieniono image na wersjonowany tag",
                                            f"image: {image}",
                                            f"image: {replacement}",
                                        )
                                    )
                    ...
                    if isinstance(security_context, dict):
                        if security_context.get("privileged") is True:
                            if container_name:
                                priv_line = _find_container_line(
                                    lines, container_name, "privileged"
                                )
                            else:
                                priv_line = _find_container_key_line_by_index(
                                    lines, idx, "privileged"
                                )
                            if priv_line:
                                errors.append(
                                    Issue(priv_line, 1, "K8S001", "Kontener privileged")
                                )
                                # Remove privileged: true
                                fixed_lines[priv_line - 1] = re.sub(
                                    r"^(\s*)privileged:\s*true.*$",
                                    r"\1# privileged: true - REMOVED",
                                    fixed_lines[priv_line - 1],
                                )
                                fixes.append(
                                    Fix(
                                        priv_line,
                                        "Usunięto privileged: true",
                                        "privileged: true",
                        ...
                        if security_context.get("runAsUser") == 0:
                            root_line = _find_container_line(
                                lines, container_name, "runAsUser"
                            )
                            if root_line:
                                warnings.append(
                                    Issue(root_line, 1, "K8S002", "Kontener jako root")
                                )
                        ...
                        if res_line:
                            warnings.append(
                                Issue(
                                    ...
                                    1,
                                    "K8S008",
                                    f"Brak resource limits dla {kind}",
                                )
                            )
                        ...
                        if probe_line:
                            warnings.append(
                                Issue(
                                    ...
                                    1,
                                    "K8S009",
                                    f"Brak liveness probe dla {kind}",
                                )
                            )
                            ...
                            insert_pos = _find_insert_position(
                                lines, probe_line, container_name
                            )
                        ...
                        if probe_line:
                            warnings.append(
                                Issue(
                                    ...
                                    1,
                                    "K8S009",
                                    f"Brak readiness probe dla {kind}",
                                )
                            )
            ...
            if not pod_spec.get("securityContext"):
                warnings.append(
                    Issue(1, 1, "K8S010", f"Brak pod-level securityContext dla {kind}")
                )
                        ...
                        if vol_line:
                            warnings.append(
                                Issue(
                                    ...
                                    1,
                                    "K8S003",
                                    "hostPath - użyj PersistentVolume",
                                )
                            )
                ...
                if "value:" in line:
                    ...
                    if any(
                        ...
                        if "${" not in line and "valueFrom:" not in line:
                            errors.append(
                                Issue(i, 1, "K8S006", "Hardcoded secret - użyj Secret")
                            )
    ...
    return AnalysisResult(
        ...
        warnings,
        fixes,
        {"kinds": [d.get("kind") for d in documents if isinstance(d, dict)]},
    )


def _find_container_key_line_by_index(
    lines: List[str], container_index: int, key: str
) -> int:
...
def _find_insert_position(
    lines: List[str], start_line: int, container_name: str
) -> int:
