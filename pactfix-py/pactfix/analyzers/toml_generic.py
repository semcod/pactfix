...
def analyze_toml(code: str) -> AnalysisResult:
    ...
    if re.search(
        r"^\s*[A-Za-z0-9_.-]+\s+[A-Za-z0-9_.-]+\s*=\s*", fixed_code, re.MULTILINE
    ):
        warnings.append(
            Issue(
                1,
                1,
                "TOML004",
                "Podejrzany klucz z odstępami przed = (sprawdź składnię TOML)",
            )
        )
    ...
    if tomllib is None:
        warnings.append(
            Issue(
                1, 1, "TOML001", "Brak tomllib (Python < 3.11) - pomijam walidację TOML"
            )
        )
