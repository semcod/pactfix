...
def _check_weak_ssl_protocols(
    ...
    if "ssl_protocols" in stripped and (
        "SSLv3" in stripped or "TLSv1 " in stripped or "TLSv1.1" in stripped
    ):
...
def _collect_server_blocks(fixed_lines: List[str]) -> List[Tuple[int, int]]:
    server_blocks = []
    brace = 0
    in_server = False
    server_start = None
    ...
    for idx, line in enumerate(fixed_lines):
        ...
        if (
            in_server
            and server_level is not None
            and brace == server_level
            and server_start is not None
            and idx > server_start
        ):
...
def _analyze_server_blocks(
    fixed_lines: List[str],
    warnings: List[Issue],
    fixes: List[Fix],
) -> None:
    server_blocks = _collect_server_blocks(fixed_lines)
    inserts: List[Tuple[int, List[str], int]] = []
    all_text = "\n".join(fixed_lines)
    has_any_ssl = bool(re.search(r"^\s*listen\s+443\b", all_text, re.MULTILINE)) or (
        "ssl_certificate" in all_text
    )
    ...
    for start, end in server_blocks:
        ...
        block_text = "\n".join(block_lines)
        has_ssl = (
            bool(re.search(r"^\s*listen\s+443\b", block_text, re.MULTILINE))
            or " ssl" in block_text
        )
        ...
        if has_http and has_any_ssl and "return 301 https://" not in block_text:
            ...
            inserts.append(
                (
                    insert_at,
                    [insert_indent + "return 301 https://$host$request_uri;"],
                    start + 1,
                )
            )
            warnings.append(
                Issue(start + 1, 1, "NGINX007", "Brak przekierowania HTTP->HTTPS")
            )
        ...
        if has_ssl and "ssl_session_tickets" not in block_text:
            ...
            insert_indent = re.match(r"^\s*", fixed_lines[start]).group(0) + "    "
            inserts.append(
                (insert_at, [insert_indent + "ssl_session_tickets off;"], start + 1)
            )
            warnings.append(
                Issue(start + 1, 1, "NGINX008", "Brak ssl_session_tickets off")
            )
        ...
        if has_ssl and "ssl_prefer_server_ciphers" not in block_text:
            ...
            inserts.append(
                (
                    insert_at,
                    [insert_indent + "ssl_prefer_server_ciphers on;"],
                    start + 1,
                )
            )
            warnings.append(
                Issue(start + 1, 1, "NGINX009", "Brak ssl_prefer_server_ciphers on")
            )
        ...
        if re.search(r"^\s*location\s+~\s*/\\.\s*\{\s*$", block_text, re.MULTILINE):
            ...
            for j in range(start, end + 1):
                ...
                if (
                    in_loc
                    and loc_level is not None
                    and brace2 == loc_level
                    ...
                    if "deny all;" not in loc_text:
                        indent = (
                            re.match(r"^\s*", fixed_lines[loc_start]).group(0) + "    "
                        )
                        inserts.append(
                            (loc_start + 1, [indent + "deny all;"], loc_start + 1)
                        )
                        warnings.append(
                            Issue(
                                loc_start + 1,
    ...
    for insert_at, new_lines, line_no in sorted(
        inserts, key=lambda x: x[0], reverse=True
    ):
        for k, nl in enumerate(new_lines):
            fixed_lines.insert(insert_at + k, nl)
        fixes.append(
            Fix(line_no, "Dodano ustawienia hardening", "", new_lines[0].strip())
        )
...
def analyze_nginx(code: str) -> AnalysisResult:
    ...
    for i, line in enumerate(lines, 1):
        stripped = line.strip()
        _check_server_tokens(stripped, line, i, warnings, fixes, fixed_lines)
        _check_autoindex(stripped, line, i, warnings, fixes, fixed_lines)
        _check_weak_ssl_protocols(stripped, line, i, errors, fixes, fixed_lines)
        _check_weak_ciphers(stripped, line, i, errors, fixes, fixed_lines)

    _analyze_server_blocks(fixed_lines, warnings, fixes)
