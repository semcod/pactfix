...
def analyze_docker_compose(code: str) -> AnalysisResult:
    ...
    try:
        ...
        return AnalysisResult(
            ...
            code,
            code,
            [Issue(1, 1, "COMPOSE999", "Invalid YAML")],
            [],
            [],
        )
    ...
    for svc_name, svc in services.items():
            ...
            if ":latest" in image or ":" not in image:
                ...
                for j in range(svc_start_line, len(lines)):
                    if re.match(r"^\s*image\s*:", lines[j]):
                        ...
                        break
                warnings.append(
                    Issue(img_line, 1, "COMPOSE001", "Użyj konkretnego tagu wersji")
                )
                replacement = image
                ...
                if re.match(r"^\s*privileged\s*:\s*true", lines[j]):
                    errors.append(
                        Issue(
                            ...
                            1,
                            "COMPOSE002",
                            "privileged: true jest niebezpieczne",
                        )
                    )
                    fixed_lines[j] = ""
                    fixes.append(
                        Fix(j + 1, "Usunięto privileged: true", lines[j].strip(), "")
                    )
                    break
        ...
        if svc.get("network_mode") == "host":
            ...
            for j in range(nm_line, len(lines)):
                if re.match(r"^\s*network_mode\s*:\s*host", lines[j]):
                    warnings.append(
                        Issue(
                            j + 1, 1, "COMPOSE003", "network_mode: host omija izolację"
                        )
                    )
                    break
                    ...
                    if "/var/run/docker.sock" in lines[j]:
                        errors.append(
                            Issue(
                                ...
                                1,
                                "COMPOSE004",
                                "Montowanie docker.sock daje pełny dostęp",
                            )
                        )
                        break
        ...
        if isinstance(env, dict):
            for k, v in env.items():
                if (
                    any(p in k.upper() for p in secret_patterns)
    ...
    if len(services) > 1 and not has_networks:
        warnings.append(
            Issue(
                1, 1, "COMPOSE006", f"Zdefiniuj networks dla {len(services)} serwisów"
            )
        )
