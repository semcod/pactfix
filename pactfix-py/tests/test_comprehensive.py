...
from pactfix.analyzer import analyze_code, detect_language, add_fix_comments
...
def run_language_check(
    language: str, fixture_file: str, expected_min: int
) -> TestResult:
...
def run_all_tests(verbose: bool = True) -> TestSuiteResult:
    ...
    for language, fixture_file in FIXTURE_FILES.items():
        ...
        if verbose:
            ...
            print(
                f"{status} ({result.errors_found}E/{result.warnings_found}W/{result.fixes_applied}F)"
            )
...
def main():
    ...
    parser.add_argument(
        "--save-report", action="store_true", help="Save report to JSON"
    )
