...
def analyze_dockerfile(code: str) -> dict:
    ...
    for i, line in enumerate(lines, 1):
        ...
        last_cmd_line = check_cmd_entrypoint_format(
            i, stripped, upper_line, warnings, last_cmd_line
        )
...
def check_base_image(
    i: int, stripped: str, upper_line: str, warnings: List[Dict], base_image: str | None
) -> str | None:
...
def extract_exposed_ports(
    stripped: str, upper_line: str, exposed_ports: Set[str]
) -> None:
...
def check_apt_get_usage(
    i: int, stripped: str, upper_line: str, warnings: List[Dict]
) -> None:
...
def check_add_vs_copy(
    i: int, stripped: str, upper_line: str, warnings: List[Dict]
) -> None:
    if (
        upper_line.startswith("ADD ")
        and "http" not in stripped
        and ".tar" not in stripped
    ):
        warnings.append(
            {
                "line": i,
...
def check_copy_with_chown(
    i: int, stripped: str, upper_line: str, warnings: List[Dict], has_user: bool
) -> None:
...
def check_cmd_entrypoint_format(
    i: int, stripped: str, upper_line: str, warnings: List[Dict], last_cmd_line: int
) -> int:
            ...
            errors.append(
                {
                    ...
                    "column": 1,
                    "code": "DOCKER007",
                    "message": "Możliwy hardcoded secret - użyj build args lub secrets",
                }
            )


def check_workdir_path(
    i: int, stripped: str, upper_line: str, warnings: List[Dict]
) -> None:
...
def add_context_based_warnings(
    has_user: bool, has_healthcheck: bool, base_image: str | None, warnings: List[Dict]
) -> None:
    if not has_user:
        warnings.append(
            {
